...
class DataLoader:
    ...
    def __init__(self, file_path, batch_size, img_size, max_text_len):
        ...
        for line in f:
            ...
            file_name = file_path + 'words/' + file_name_split[0] + '/' + file_name_split[0] + '-' + file_name_split[1] + '/' + \
                       line_split[0] + '.png'
    ...
    def get_next(self):
        ...
        images = [
            ImagePreprocessor.preprocess(cv2.imread(self.samples[i].filePath, cv2.IMREAD_GRAYSCALE), self.imgSize, self.dataAugmentation)
